...
export default function Checkin() {
  ...
  const spinColor = RC ? RC.color : C.geo;
  const spinBg    = RC ? RC.glow  : C.geoDim;
    ...
    <div style={{
      ...
      {showWave && (
        <div style={{
          ...
          background: `linear-gradient(0deg, rgba(143,174,123,0.18) 0%, rgba(143,174,123,0.06) 60%, transparent 100%)`,
          ...
          <form onSubmit={handlePinSubmit}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${pinError ? C.red : pin.length >= 4 ? 'rgba(201,123,71,0.45)' : C.b2}`,
            ...
            <button type="submit" disabled={pin.length < 4} style={{
              width: '100%',
              background: pin.length >= 4 ? C.geo : C.cardHi,
              color: pin.length >= 4 ? C.bg : C.t3,
      ...
      {status === 'success' && (() => {
        ...
        const accentColor = isGeohunt ? C.gold : isPromo && RC ? RC.color : C.geo;
        const accentBg    = isGeohunt ? C.goldFt : isPromo && RC ? RC.bg : C.geoDim;
        const accentBorder= isGeohunt ? C.goldGl  : isPromo && RC ? RC.color + '30' : C.geoGl;

-- ============================================================
-- GeoEarn — Обязательные миграции перед запуском
-- Запускать В SUPABASE SQL EDITOR целиком (одним блоком)
-- ============================================================
...
-- Гарантируем наличие единственной строки
INSERT INTO platform_wallet (id, balance) VALUES (1, 0) ON CONFLICT DO NOTHING;


-- ── 2. Platform transactions (аудит комиссий) ──────────────────────────────────
...
-- ── 6. Колонки users — геймификация и безопасность ────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_geo_rate_history_created_at ON geo_rate_history (created_at DESC);


-- ── 9. Superadmin audit log ────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_sa_audit_log_created_at ON sa_audit_log (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_sa_audit_log_action     ON sa_audit_log (action);
CREATE INDEX IF NOT EXISTS idx_sa_audit_log_target_id  ON sa_audit_log (target_id) WHERE target_id IS NOT NULL;


-- ── 10. Verification pins ──────────────────────────────────────────────────────
...
-- ── 12. Геймификация: стрики, XP, задачи, достижения ─────────────────────────
...
CREATE TABLE IF NOT EXISTS referral_earnings (
  ...
  visit_id    INTEGER REFERENCES visits(id) ON DELETE CASCADE, -- nullable: activation bonus не имеет visit_id
  amount      INTEGER NOT NULL,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_ref_earnings_referrer ON referral_earnings (referrer_id, created_at DESC);

-- Если таблица уже существует со старым NOT NULL — исправляем
...
-- ── Финальная проверка ────────────────────────────────────────────────────────
SELECT 'platform_wallet' AS check, balance::TEXT AS value,
  CASE WHEN balance >= 0 THEN '✅ OK' ELSE '🔴 ERROR' END AS status
FROM platform_wallet
UNION ALL
SELECT 'task_definitions', COUNT(*)::TEXT, '✅ ' || COUNT(*) || ' задач' FROM task_definitions
UNION ALL
SELECT 'achievement_definitions', COUNT(*)::TEXT, '✅ ' || COUNT(*) || ' достижений' FROM achievement_definitions;

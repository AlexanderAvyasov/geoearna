...
async function performCheckin({ userId, qrToken, lat, lng }) {
  const { data: business, error: businessError, status: businessStatus } = await supabase
    .from('businesses')
    .select('*')
    .eq('qr_token', qrToken)
    .maybeSingle();
  ...
  const campaign = (campaigns || []).find((item) => {
    ...
    const campaignActive = !endsAt || endsAt > now;
    return campaignActive && item.visits_count < item.max_visits;
  });
